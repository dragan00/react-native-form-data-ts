...
interface Props {
  currentTime: number;
  duration: number;
  onSlideCapture: (data: { currentTime: number, seekTime: number}) => void;
  onSlideStart: () => void;
...
const ProgressBar: React.FC<Props> = ({
  ...
  return (
    <View style={styles.wrapper}>
      <Slider
        ...
        minimumTrackTintColor={'#4675FF'}
        maximumTrackTintColor={'#FFFFFF'}
        thumbTintColor={'#4675FF'}
      />
  ...
  function handleOnSlide(time: number) {
    onSlideCapture({ currentTime: time + 1, seekTime: time});
  }
};
